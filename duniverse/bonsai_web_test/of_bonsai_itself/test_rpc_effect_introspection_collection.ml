  ...
  module Polling_state_rpc = struct
    module Response = struct
      ...
      let diffs ~from ~to_ =
        ...
        match String.equal from to_ with
        | true -> []
        | false -> [ to_ ]
      ;;
    end
...
let consume_and_apply_events
  ...
  in
  if should_print_table then print_table () else print_diff ();
  new_state
;;
...
module Dispatcher_handle = struct
  ...
  let create ?rpc_implementations ~sexp_of_response ~dispatch () =
    let handle =
      Handle.create
        ?rpc_implementations
...
module%test [@name "Normal Rpc.Rpc.dispatch"] _ = struct
  ...
  let%expect_test "dispatching a normal rpc - rpc does not exist error" =
    let { Dispatcher_handle.handle; state } =
      create_handle ~sexp_of_response:sexp_of_string ()
    in
...
module%test [@name "Rpc_effect.Rpc.streamable_dispatcher"] _ = struct
  let create_handle ?rpc_implementations () =
    Dispatcher_handle.create
      ~sexp_of_response:sexp_of_string
      ?rpc_implementations
...
module%test [@name "Rpc_effect.Polling_state_rpc.dispatcher"] _ = struct
  let create_handle ?rpc_implementations () =
    Dispatcher_handle.create
      ~sexp_of_response:sexp_of_string
      ?rpc_implementations
  ...
  let%expect_test "Indexing is correct - empty diff" =
    let { Dispatcher_handle.handle; state } =
      create_handle
        ~rpc_implementations:[ polling_state_rpc_reverse_rpc_implementation ]
        ()
    ...
    [%expect
      {|
      ("RPC started" (query capybara))
      ("Computing diff" (from arabypac) (to_ arabypac))
      ("RPC finished" (response (Ok arabypac)))
      |}];
    let _state = consume_and_apply_events ~state () in
    (* NOTE: Importantly [(Update ())] expresses that an empty diff occurred. *)
    [%expect
      {|
      Events
      =========================
...
module%test [@name "Normal Rpc.Rpc.poll"] _ = struct
  let create_handle ?rpc_implementations () =
...
module%test [@name "Rpc_effect.Rpc.babel_poll and babel_poll_until_ok"] _ = struct
  let create_handle ?rpc_implementations () =
    Poller_handle.create
      ?rpc_implementations
      ~poller:
...
module%test [@name "Rpc_effect.Polling_state_rpc.poll and babel_poll"] _ = struct
  let create_handle ?rpc_implementations () =
    Poller_handle.create
      ?rpc_implementations
      ~poller:
...
module%test [@name "Rpc_effect.Rpc.streamable_poll"] _ = struct
  let create_handle ?rpc_implementations () =
    Poller_handle.create
      ?rpc_implementations
      ~poller:
...
module%test [@name "More interesting polling_state_rpc diffs"] _ = struct
  module Polling_state_rpc = struct
    module Response = struct
      type t =
        { name : string
        ; capybaras : string String.Map.t
        }
      [@@deriving bin_io, sexp, diff]
    end

    module Psrpc_response =
      Diffable_polling_state_rpc_response.Polling_state_rpc_response.Make (Response)

    let rpc =
      Polling_state_rpc.create
        ~name:"polling-state-rpc-more-interesting-diff"
        ~version:1
        ~query_equal:[%equal: string]
        ~bin_query:bin_string
        (module Psrpc_response)
    ...
    let polling_state_rpc_implementation =
      let m name res = name, { Response.name; capybaras = String.Map.of_alist_exn res } in
      let normal = m "normal" [ "capy1", "capy1"; "capy2", "capy2" ] in
      let responses =
        String.Map.of_alist_exn
          [ normal
          ; m "capy-alpacas" [ "capy1", "alpaca1"; "capy2", "alpaca2" ]
          ; m
              "capy-alpacas-single-add"
              [ "capy1", "alpaca1"; "capy2", "alpaca2"; "capy3", "alpaca3" ]
          ]
      in
      Rpc.Implementation.lift
        ~f:(fun connection -> connection, connection)
        (Polling_state_rpc.implement
           ...
           (fun _ string ->
             let out =
               match Map.find responses string with
               | None -> snd normal
               | Some x -> x
             in
             return out)
  ...
  let%expect_test "Showing ppx_diff diffs" =
    let { Dispatcher_handle.handle; state } =
      create_handle
        ~rpc_implementations:[ Polling_state_rpc.polling_state_rpc_implementation ]
        ()
    in
    Handle.do_actions handle [ Send_rpc "normal" ];
    let%bind () = Async_kernel_scheduler.yield_until_no_jobs_remain () in
    [%expect
      {|
      ("RPC started" (query normal))
      ("RPC finished"
       (response (Ok ((name normal) (capybaras ((capy1 capy1) (capy2 capy2)))))))
      |}];
    let consume_and_apply_events ~state () =
      consume_and_apply_events ~print_events:false ~should_print_table:true ~state ()
    in
    let state = consume_and_apply_events ~state () in
    [%expect
      {|
      ...
      │ rpc_id │ status                                                                    │
      ├────────┼───────────────────────────────────────────────────────────────────────────┤
      │ 0      │ (Finished (duration 0s)                                                   │
      │        │  (response                                                                │
      │        │   (Ok                                                                     │
      │        │    (Sexp_of_provided                                                      │
      │        │     (Fresh ((name normal) (capybaras ((capy1 capy1) (capy2 capy2))))))))) │
      └────────┴───────────────────────────────────────────────────────────────────────────┘
      |}];
    (* No change in query -> empty diffs. *)
    Handle.do_actions handle [ Send_rpc "normal" ];
    let%bind () = Async_kernel_scheduler.yield_until_no_jobs_remain () in
    [%expect
      {|
    ...
    [%expect
      ...
      │        │     (Fresh ((name normal) (capybaras ((capy1 capy1) (capy2 capy2))))))))) │
      ├────────┼───────────────────────────────────────────────────────────────────────────┤
      │ 1      │ (Finished (duration 0s) (response (Ok (Sexp_of_provided (Update ())))))   │
      └────────┴───────────────────────────────────────────────────────────────────────────┘
      |}];
    (* Change in query -> response gets diffed via ppx_diff. *)
    Handle.do_actions handle [ Send_rpc "capy-alpacas" ];
    let%bind () = Async_kernel_scheduler.yield_until_no_jobs_remain () in
    [%expect
      {|
      ("RPC started" (query capy-alpacas))
      ("RPC finished"
       (response
        (Ok ((name capy-alpacas) (capybaras ((capy1 alpaca1) (capy2 alpaca2)))))))
      |}];
    let state = consume_and_apply_events ~state () in
    [%expect
      {|
      ...
      │ 1      │ (Finished (duration 0s) (response (Ok (Sexp_of_provided (Update ())))))   │
      ├────────┼───────────────────────────────────────────────────────────────────────────┤
      │ 2      │ (Finished (duration 0s)                                                   │
      │        │  (response                                                                │
      │        │   (Ok                                                                     │
      │        │    (Sexp_of_provided                                                      │
      │        │     (Update                                                               │
      │        │      (((Name capy-alpacas)                                                │
      │        │        (Capybaras ((Diff capy2 alpaca2) (Diff capy1 alpaca1))))))))))     │
      └────────┴───────────────────────────────────────────────────────────────────────────┘
      |}];
    (* Change in query -> response gets diffed via ppx_diff. *)
    Handle.do_actions handle [ Send_rpc "capy-alpacas-single-add" ];
    let%bind () = Async_kernel_scheduler.yield_until_no_jobs_remain () in
    [%expect
      {|
      ("RPC started" (query capy-alpacas-single-add))
      ("RPC finished"
       (response
        (Ok
         ((name capy-alpacas-single-add)
          (capybaras ((capy1 alpaca1) (capy2 alpaca2) (capy3 alpaca3)))))))
      |}];
    let state = consume_and_apply_events ~state () in
    [%expect
      {|
      ...
      │        │        (Capybaras ((Diff capy2 alpaca2) (Diff capy1 alpaca1))))))))))          │
      ├────────┼────────────────────────────────────────────────────────────────────────────────┤
      │ 3      │ (Finished (duration 0s)                                                        │
      │        │  (response                                                                     │
      │        │   (Ok                                                                          │
      │        │    (Sexp_of_provided                                                           │
      │        │     (Update                                                                    │
      │        │      (((Name capy-alpacas-single-add) (Capybaras ((Add capy3 alpaca3)))))))))) │
      └────────┴────────────────────────────────────────────────────────────────────────────────┘
      |}];
    Handle.do_actions handle [ Send_rpc "capy-alpacas" ];
    let%bind () = Async_kernel_scheduler.yield_until_no_jobs_remain () in
    ...
    [%expect
      ...
      │        │      (((Name capy-alpacas-single-add) (Capybaras ((Add capy3 alpaca3)))))))))) │
      ├────────┼────────────────────────────────────────────────────────────────────────────────┤
      │ 4      │ (Finished (duration 0s)                                                        │
      │        │  (response                                                                     │
      │        │   (Ok                                                                          │
      │        │    (Sexp_of_provided                                                           │
      │        │     (Update (((Name capy-alpacas) (Capybaras ((Remove capy3))))))))))          │
      └────────┴────────────────────────────────────────────────────────────────────────────────┘
      |}];
    return ()
  ;;
...
module%test
  [@name "Rpc_effect.Rpc.poll_until_condition_met and babel_poll_until_condition_met"] _ =
struct
  let create_until_condition_met_handle ?rpc_implementations () =
    Poller_handle.create
      ?rpc_implementations
      ~poller:
        (Rpc_effect.Rpc.poll_until_condition_met
           ~condition:
             (Value.return (fun response ->
                if String.length response > 0 then `Stop_polling else `Continue))
           ~sexp_of_query:[%sexp_of: string]
           ~sexp_of_response:[%sexp_of: string]
           Rpcs.reverse_rpc_v1
           ~equal_query:[%equal: string]
  ...
  let%expect_test "Rpc_effect.Rpc.poll_until_condition_met" =
    let { Poller_handle.handle; state } =
      create_until_condition_met_handle
        ~rpc_implementations:[ reverse_rpc_implementation ]
        ()
    in
    Handle.do_actions handle [ Set_query "capybara" ];
  ...
  let create_babel_until_condition_met_handle ?rpc_implementations () =
    Poller_handle.create
      ?rpc_implementations
      ~poller:
        (Rpc_effect.Rpc.babel_poll_until_condition_met
           ~condition:
             (Value.return (fun response ->
                if String.length response > 0 then `Stop_polling else `Continue))
           ~sexp_of_query:[%sexp_of: string]
  ...
  let%expect_test "Rpc_effect.Rpc.babel_poll_until_condition_met" =
    let { Poller_handle.handle; state } =
      create_babel_until_condition_met_handle
        ~rpc_implementations:[ reverse_rpc_implementation ]
        ()
    in
    Handle.do_actions handle [ Set_query "capybara" ];
