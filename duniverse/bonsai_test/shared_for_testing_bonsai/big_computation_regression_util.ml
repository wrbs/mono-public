...
module For_cont = struct
  ...
  let basic : height:int -> width:int -> local_ Bonsai.graph -> String.Set.t Bonsai.t =
    fun ~height ~width ->
    Fn.apply_n_times
      ~n:height
      (fun rem (local_ graph) ->
  ...
  let with_assoc
    :  height:int -> width:int -> num_assocs:int -> local_ Bonsai.graph
  ...
  let with_switch : height:int -> width:int -> local_ Bonsai.graph -> int Bonsai.t =
