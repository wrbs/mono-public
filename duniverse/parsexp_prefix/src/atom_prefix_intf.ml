...
module Signified = struct
  type t =
    ...
    (** [Complete] means [prefix] is all the signified from the start of the atom to the
        end of the input seen so far. *)
...
module type Atom_prefix = sig
  ...
  (** [get_signifier t ~parser_input] returns the substring of [parser_input] this [t]
      corresponds to, starting at the beginning of the partial atom, ending at the
      position the parser was at when [create] was called.

      [parser_input] should contain the entire document being parsed (or at least the
      prefix that was fed to the parser so far).
