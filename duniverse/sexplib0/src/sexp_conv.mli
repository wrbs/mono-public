...
val list_map__local : local_ (local_ 'a -> local_ 'b) -> local_ 'a list -> local_ 'b list
...
val sexp_of_unit__local : local_ unit -> local_ Sexp.t
...
val sexp_of_bool__local : local_ bool -> local_ Sexp.t
...
val sexp_of_string__local : local_ string -> local_ Sexp.t
...
val sexp_of_bytes__local : local_ bytes -> local_ Sexp.t
...
val sexp_of_char__local : local_ char -> local_ Sexp.t
...
val sexp_of_int__local : local_ int -> local_ Sexp.t
...
(** As [sexp_of_float], but returning a locally-allocated sexp. Currently, the float will
    ...
    heap-allocated. *)
val sexp_of_float__local : local_ float -> local_ Sexp.t
...
(** As [sexp_of_int32], but returning a locally-allocated sexp. Currently, the sexp will
    contain a formatted string which is heap-allocated. *)
val sexp_of_int32__local : local_ int32 -> local_ Sexp.t
...
(** As [sexp_of_int64], but returning a locally-allocated sexp. Currently, the sexp will
    contain a formatted string which is heap-allocated. *)
val sexp_of_int64__local : local_ int64 -> local_ Sexp.t
...
val sexp_of_nativeint__local : local_ nativeint -> local_ Sexp.t
...
val sexp_of_ref__local : (local_ 'a -> local_ Sexp.t) -> local_ 'a ref -> local_ Sexp.t
...
val sexp_of_or_null__local
  :  (local_ 'a -> local_ Sexp.t)
  -> local_ 'a Or_null_shim.t
  -> local_ Sexp.t
...
val sexp_of_list__local : (local_ 'a -> local_ Sexp.t) -> local_ 'a list -> local_ Sexp.t
