...
(** An adsr envelope. Returns [value_opt, send_event].
  ...
  The reason it returns [None] if the envelope is finished rather than a block
  of all [0.]s is so you can [match%sub] on it to disable computations that will
  be silenced when there isn't a note playing.  *)
val adsr
