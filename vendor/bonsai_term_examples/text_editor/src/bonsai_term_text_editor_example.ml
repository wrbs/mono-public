...
let app ?(initial_keybindings = `Standard) ~dimensions (local_ graph) =
  ...
  let view =
    ...
    let bar =
      ...
      let keybindings =
        ...
        in
        add_click_handler node ~on_click:(toggle ())
      in
  ...
  let toggle_keybindings_mode =
    ...
    toggle ()
  in
  ~view, ~handler, ~toggle_keybindings_mode, ~text, ~set_text, ~mode, ~get_cursor_position
;;

let register_cursor ~view ~mode ~get_cursor_position (graph @ local) =
  let update_cursor_position =
    ...
    | Some { Position.x; y } ->
      set_cursor (Some { position = { x; y }; kind = cursor_kind })
  in
