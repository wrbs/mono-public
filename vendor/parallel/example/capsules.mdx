...
API](https://ocaml-docs/basement/Basement/Capsule/index.html).  A
more ergonomic, work-in-progress API is available in
[`Portable.Capsule`](https://ocaml-docs/portable_kernel/Portable_kernel/Capsule/index.html)._
...
```ocaml
# let capsule_ref = Capsule.Data.create (fun () -> ref 0)
val capsule_ref : (int ref, '_weak2) Capsule.Data.t = <abstr>
```
...
```ocaml
# let initial = Capsule.initial
val initial : Capsule.initial Capsule.Access.boxed = <abstr>
```
...
let increment_initial () =
  ...
  increment initial_ref ~access:(Capsule.Access.unbox initial)
```
...
Error: This expression has type (int ref, 'k) Capsule.Data.t
       but an expression was expected of type (int ref, $k) Capsule.Data.t
       The type constructor $k would escape its scope
       Hint: $k is an existential type bound by the constructor P.
```
...
let parallel_increment parallel ~(password : 'k Capsule.Password.t @ local) =
  ...
  Parallel.fork_join2
    ...
    (fun _ -> increment ~password capsule_ref) [@nontail]
```
```mdx-error
Line 6, characters 28-36:
Error: The value password is local but is expected to be global
...
let increment_fresh () =
  ...
  let _ : _ =
    Capsule.Key.with_password key ~f:(fun password ->
      increment ~password fresh_ref [@nontail])
  in
  ()
```
...
let parallel_keys parallel =
  ...
  let _ : _ =
    Parallel.fork_join2
      parallel
      (fun _ -> let #(_, _) = Capsule.Key.with_password key ~f:(fun _ -> ()) in ())
      (fun _ -> let #(_, _) = Capsule.Key.with_password key ~f:(fun _ -> ()) in ())
  in
  ()
```
