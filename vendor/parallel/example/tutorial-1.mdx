...
(that is, it [crosses contention](#mode-crossing)). Do go over the documentation in the
`Atomic` module,
as it has many useful operations,
...
We first need to convert from `iarray` to `Parallel.Sequence.t`,
a general sequence type supporting a rich selection of parallel operations.
Among them is a parallel `reduce`, which operates much like an unordered `fold`.
