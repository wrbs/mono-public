...
module Latte = struct
  ...
  let color = memo (fun t -> Rgb.to_color (rgb t))
end
...
let variable =
  Bonsai.Dynamic_scope.create ~name:"catppuccin-flavor" ~fallback:Flavor.Mocha ()
;;
...
let set_flavor_within flavor inside = Bonsai.Dynamic_scope.set variable flavor ~inside

let set_flavor_within_app flavor inside (local_ graph) =
  let open Bonsai.Let_syntax in
  let%sub view, handler =
    Bonsai.Dynamic_scope.set variable flavor graph ~inside:(fun (local_ graph) ->
      let ~view, ~handler = inside graph in
      Bonsai.both view handler)
  in
  let view =
    let%arr view and flavor in
...
let rgb flavor color =
  match flavor with
  | Flavor.Latte -> Mocha.rgb color
  | Frappe -> Frappe.rgb color
  | Macchiato -> Macchiato.rgb color
  | Mocha -> Mocha.rgb color
;;
