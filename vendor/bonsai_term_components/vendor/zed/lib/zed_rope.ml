...
type t =
  | Leaf of Zed_utf8.t * int (* [Leaf(str, len)] *)
...
let rec make_fibo acc a b =
  let c = a + b in
  if c < b then (* overflow *)
             acc else make_fibo (c :: acc) b c
;;
...
let unsafe_concat rope1 rope2 =
  ...
  | _, Leaf (_, 0) -> rope1
  | _ ->
    Node (1 + max (depth rope1) (depth rope2), length rope1, rope1, length rope2, rope2)
;;

let rec insert_to_forest forest acc idx =
  let acc = unsafe_concat forest.(idx) acc in
  if length acc < fibo.(idx + 1)
  then forest.(idx) <- acc
  else (
    forest.(idx) <- empty;
...
let rec concat_forest_until forest acc idx rope =
  if length rope < fibo.(idx + 1)
  then insert_to_forest forest (unsafe_concat acc rope) idx
  else (
    let acc = unsafe_concat forest.(idx) acc in
    forest.(idx) <- empty;
...
let rec balance_rec forest rope =
  match rope with
  | Leaf _ -> concat_forest_until forest empty 2 rope
...
let rec concat_forest forest acc idx =
  if idx = max_depth
  then acc
  else concat_forest forest (unsafe_concat forest.(idx) acc) (idx + 1)
;;

let balance rope =
  match length rope with
  | 0 | 1 -> rope
  | len when len >= fibo.(depth rope + 2) -> rope
  | _ ->
...
let append rope1 rope2 =
  ...
  | Node (d, len_l, rope_l, _, Leaf (text1, len1)), Leaf (text2, len2)
    when len1 + len2 <= max_leaf_size ->
    Node (d, len_l, rope_l, len1 + len2, Leaf (text1 ^ text2, len1 + len2))
  | Leaf (text1, len1), Node (d, _, Leaf (text2, len2), len_r, rope_r)
    when len1 + len2 <= max_leaf_size ->
    Node (d, len1 + len2, Leaf (text1 ^ text2, len1 + len2), len_r, rope_r)
  | _ ->
    balance
      (Node (1 + max (depth rope1) (depth rope2), length rope1, rope1, length rope2, rope2))
;;
...
let rec unsafe_get idx rope =
  match rope with
  | Leaf (text, _) -> Zed_utf8.get text idx
  | Node (_, len_l, rope_l, _, rope_r) ->
    if idx < len_l then unsafe_get idx rope_l else unsafe_get (idx - len_l) rope_r
;;

let get rope idx =
  if idx < 0 || idx >= length rope then raise Out_of_bounds else unsafe_get idx rope
;;

let rec unsafe_sub rope idx len =
  match rope with
  | Leaf (text, _) -> Leaf (Zed_utf8.sub text idx len, len)
  | Node (_, len_l, rope_l, len_r, rope_r) ->
    if len = len_l + len_r
    then rope
    else if idx >= len_l
    then unsafe_sub rope_r (idx - len_l) len
    else if idx + len <= len_l
    then unsafe_sub rope_l idx len
    else
      append
...
let sub rope idx len =
  if idx < 0 || len < 0 || idx + len > length rope
  then raise Out_of_bounds
  else unsafe_sub rope idx len
;;

let make length char =
  if length < max_leaf_size
  then Leaf (Zed_utf8.make length char, length)
  else (
    ...
    let rec loop acc n =
      if n = 0
      then acc
      else if n < max_leaf_size
      then append acc (Leaf (Zed_utf8.sub text 0 n, n))
      else loop (append acc chunk) (n - max_leaf_size)
    in
    loop empty length)
;;

let singleton ch = Leaf (Zed_utf8.singleton ch, 1)
...
let after rope pos = sub rope pos (length rope - pos)
...
let rec iter f = function
  | Leaf (text, _) -> Zed_utf8.iter f text
  | Node (_, _, rope_l, _, rope_r) ->
    iter f rope_l;
    iter f rope_r
;;
...
let rec rev_iter f = function
  | Leaf (text, _) -> Zed_utf8.rev_iter f text
  | Node (_, _, rope_l, _, rope_r) ->
    rev_iter f rope_r;
    rev_iter f rope_l
;;
...
let rec fold f rope acc =
  match rope with
  | Leaf (text, _) -> Zed_utf8.fold f text acc
  | Node (_, _, rope_l, _, rope_r) -> fold f rope_r (fold f rope_l acc)
;;

let rec rev_fold f rope acc =
  match rope with
  | Leaf (text, _) -> Zed_utf8.rev_fold f text acc
  | Node (_, _, rope_l, _, rope_r) -> rev_fold f rope_l (rev_fold f rope_r acc)
;;

let rec map f = function
  | Leaf (txt, len) -> Leaf (Zed_utf8.map f txt, len)
...
let rec rev_map f = function
  | Leaf (txt, len) -> Leaf (Zed_utf8.rev_map f txt, len)
  | Node (depth, length_l, rope_l, length_r, rope_r) ->
...
let rec fold_leaf f rope acc =
  match rope with
  | Leaf (text, _) -> f text acc
  | Node (_, _, rope_l, _, rope_r) -> fold_leaf f rope_r (fold_leaf f rope_l acc)
;;

let rec rev_fold_leaf f rope acc =
  match rope with
  | Leaf (text, _) -> f text acc
  | Node (_, _, rope_l, _, rope_r) -> rev_fold_leaf f rope_l (rev_fold_leaf f rope_r acc)
;;
...
module Zip = struct
  ...
  let rec move_utf8_f str idx len =
    if len = 0 then idx else move_utf8_f str (Zed_utf8.unsafe_next str idx) (len - 1)
  ;;
  ...
  let rec make_f_rec ofs rope pos rest_b rest_f =
    ...
    | Node (_, _, r1, _, r2) ->
      ...
      if pos < len1
      then make_f_rec ofs r1 pos rest_b (r2 :: rest_f)
      else make_f_rec ofs r2 (pos - len1) (r1 :: rest_b) rest_f
  ;;
  ...
  let rec move_utf8_b str idx len =
    if len = 0 then idx else move_utf8_b str (Zed_utf8.unsafe_prev str idx) (len - 1)
  ;;
  ...
  let rec make_b_rec ofs rope pos rest_b rest_f =
    ...
    | Node (_, _, r1, _, r2) ->
      let len1 = length r1 in
      if pos < len1
      then make_b_rec ofs r1 pos rest_b (r2 :: rest_f)
      else make_b_rec ofs r2 (pos - len1) (r1 :: rest_b) rest_f
  ;;
  ...
  let offset zip = zip.zip.ofs + zip.pos

  let rec next_leaf ofs rope rest_b rest_f =
    match rope with
    | Leaf (str, _) ->
      let chr, idx = Zed_utf8.unsafe_extract_next str 0 in
      chr, { idx; pos = 1; zip = { str; ofs; leaf = rope; rest_b; rest_f } }
    | Node (_, _, r1, _, r2) -> next_leaf ofs r1 rest_b (r2 :: rest_f)
  ;;

  let next zip =
    if zip.idx = String.length zip.zip.str
    then (
      match zip.zip.rest_f with
      | [] -> raise Out_of_bounds
      | rope :: rest ->
        next_leaf
          (zip.zip.ofs + length zip.zip.leaf)
          rope
  ...
  let rec prev_leaf ofs rope rest_b rest_f =
    ...
    | Node (_, _, r1, _, r2) -> prev_leaf ofs r2 (r1 :: rest_b) rest_f
  ;;
  ...
  let prev zip =
    ...
    then (
      ...
      | [] -> raise Out_of_bounds
      | rope :: rest -> prev_leaf zip.zip.ofs rope rest (zip.zip.leaf :: zip.zip.rest_f))
    else (
  ...
  let rec move_f n ofs rope rest_b rest_f =
    ...
    | Leaf (str, len) ->
      ...
      then
        ...
        match rest_f with
        | [] -> raise Out_of_bounds
        | rope' :: rest_f -> move_f (n - len) (ofs + len) rope' (rope :: rest_b) rest_f)
    | Node (_, _, r1, _, r2) -> move_f n ofs r1 rest_b (r2 :: rest_f)
  ;;
  ...
  let rec move_b n ofs rope rest_b rest_f =
    ...
    | Leaf (str, len) ->
      ...
      then
        ...
        match rest_b with
        | [] -> raise Out_of_bounds
        | rope' :: rest_b -> move_b (n - len) (ofs - len) rope' rest_b (rope :: rest_f))
    | Node (_, _, r1, _, r2) -> move_b n ofs r2 (r1 :: rest_b) rest_f
  ;;
  ...
  let move n zip =
    ...
    then (
      ...
      if zip.pos + n <= len
      then { zip with idx = move_utf8_f zip.zip.str zip.idx n; pos = zip.pos + n }
      else (
        match zip.zip.rest_f with
        | [] -> raise Out_of_bounds
        | rope :: rest_f ->
    ...
    then { zip with idx = move_utf8_b zip.zip.str zip.idx (-n); pos = zip.pos + n }
    else (
      match zip.zip.rest_b with
      | [] -> raise Out_of_bounds
      | rope :: rest_b ->
        move_b (n - zip.pos) zip.zip.ofs rope rest_b (zip.zip.leaf :: zip.zip.rest_f))
  ;;
  ...
  let rec sub_rec acc ropes len =
    match ropes with
    | [] -> if len > 0 then raise Out_of_bounds else acc
    | rope :: rest ->
      let len' = length rope in
      if len <= len'
      then append acc (sub rope 0 len)
      else sub_rec (append acc rope) rest (len - len')
  ;;
  ...
  let sub zip len =
    ...
    then raise Out_of_bounds
      ...
      then
        Leaf
          ( unsafe_sub zip.zip.str zip.idx (move_utf8_f zip.zip.str zip.idx len - zip.idx)
          , len )
      else
        sub_rec
          (Leaf
             (unsafe_sub zip.zip.str zip.idx (String.length zip.zip.str - zip.idx), len'))
          zip.zip.rest_f
          (len - len'))
  ;;

  let slice zip1 zip2 =
    let ofs1 = offset zip1
    and ofs2 = offset zip2 in
    if ofs1 <= ofs2 then sub zip1 (ofs2 - ofs1) else sub zip2 (ofs1 - ofs2)
  ;;
  ...
  let rec find_f f zip =
    ...
    then zip
      ...
      if f ch then zip else find_f f zip')
  ;;
  ...
  let rec find_b f zip =
    ...
    then zip
      ...
      if f ch then zip else find_b f zip')
  ;;
end
...
let rec cmp_loop str1 ofs1 str2 ofs2 rest1 rest2 =
  if ofs1 = String.length str1
  then (
    match rest1 with
    | [] -> if ofs2 = String.length str2 && rest2 = [] then 0 else -1
    | rope1 :: rest1 -> cmp_search1 rope1 str2 ofs2 rest1 rest2)
  else if ofs2 = String.length str2
  then (
    match rest2 with
    | [] -> 1
    | rope2 :: rest2 -> cmp_search2 rope2 str1 ofs1 rest1 rest2)
  else (
    let chr1, ofs1 = Zed_utf8.unsafe_extract_next str1 ofs1
    and chr2, ofs2 = Zed_utf8.unsafe_extract_next str2 ofs2 in
    let d = Zed_char.code chr1 - Zed_char.code chr2 in
    if d = 0 then cmp_loop str1 ofs1 str2 ofs2 rest1 rest2 else d)

and cmp_search1 rope1 str2 ofs2 rest1 rest2 =
  match rope1 with
  | Leaf (str1, _) -> cmp_loop str1 0 str2 ofs2 rest1 rest2
  | Node (_, _, rope1_l, _, rope1_r) ->
    cmp_search1 rope1_l str2 ofs2 (rope1_r :: rest1) rest2

and cmp_search2 rope2 str1 ofs1 rest1 rest2 =
  match rope2 with
  | Leaf (str2, _) -> cmp_loop str1 ofs1 str2 0 rest1 rest2
...
let rec cmp_init rope1 rope2 rest1 =
  match rope1 with
  | Leaf (str1, _) -> cmp_search2 rope2 str1 0 rest1 []
  | Node (_, _, rope1_l, _, rope1_r) -> cmp_init rope1_l rope2 (rope1_r :: rest1)
;;
...
module Buffer = struct
  ...
  let create () = { acc = empty; buf = String_buffer.create 1024; idx = 0 }

  let add buffer x =
    if buffer.idx = max_leaf_size
    then (
      buffer.acc
      <- append buffer.acc (Leaf (String_buffer.contents buffer.buf, max_leaf_size));
  ...
  let contents buffer =
    if buffer.idx = 0
    then buffer.acc
    else append buffer.acc (Leaf (String_buffer.contents buffer.buf, buffer.idx))
  ;;
...
let rec byte_length rope acc =
  match rope with
  | Leaf (text, _) -> acc + String.length text
  | Node (_, _, rope_l, _, rope_r) -> byte_length rope_r (byte_length rope_l acc)
;;
...
let rec blit_rope str ofs rope =
  ...
  | Leaf (text, _) ->
    ...
    ofs + len
  | Node (_, _, rope_l, _, rope_r) -> blit_rope str (blit_rope str ofs rope_l) rope_r
;;
...
let sexp_of_t t = Sexplib.Std.sexp_of_string (to_string t)
let t_of_sexp s = of_string (Sexplib.Std.string_of_sexp s)
