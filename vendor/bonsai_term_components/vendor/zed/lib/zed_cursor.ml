...
type action =
  | User_move of int
  | Text_modification of (int * int * int)
(* start, added, removed *)
...
let create length changes get_lines position wanted_column =
  ...
  let update_position position action =
    ...
    | Text_modification (start, added, removed) ->
      ...
      then
        if delta >= 0
        then (* Text has been inserted, advance the cursor. *)
          position + delta
...
let move cursor ?(set_wanted_column = true) delta =
  ...
  if new_position < 0 || new_position > !(cursor.length)
...
let goto cursor ?(set_wanted_column = true) position =
  if position < 0 || position > !(cursor.length)
  then raise Out_of_bounds
  else (
