...
let next_error s i =
  ...
  let rec main i ulen =
    ...
    then i, ulen, ""
      ...
      match ch with
      | '\x00' .. '\x7f' -> main (i + 1) (ulen + 1)
      | '\xc0' .. '\xdf' ->
        if i + 1 >= len
        then i, ulen, "premature end of UTF8 sequence"
        else (
          let byte1 = Char.code (String.unsafe_get s (i + 1)) in
          if byte1 land 0xc0 != 0x80
          then i, ulen, "malformed UTF8 sequence"
          else if ((Char.code ch land 0x1f) lsl 6) lor (byte1 land 0x3f) < 0x80
          then i, ulen, "overlong UTF8 sequence"
          else main (i + 2) (ulen + 1))
      | '\xe0' .. '\xef' ->
        if i + 2 >= len
        then i, ulen, "premature end of UTF8 sequence"
        else (
          ...
          if byte1 land 0xc0 != 0x80
          then i, ulen, "malformed UTF8 sequence"
          else if byte2 land 0xc0 != 0x80
          then i, ulen, "malformed UTF8 sequence"
          else if ((Char.code ch land 0x0f) lsl 12)
                  lor ((byte1 land 0x3f) lsl 6)
                  lor (byte2 land 0x3f)
                  < 0x800
          then i, ulen, "overlong UTF8 sequence"
          else main (i + 3) (ulen + 1))
      | '\xf0' .. '\xf7' ->
        if i + 3 >= len
        then i, ulen, "premature end of UTF8 sequence"
        else (
          ...
          then i, ulen, "malformed UTF8 sequence"
          else if byte3 land 0xc0 != 0x80
          then i, ulen, "malformed UTF8 sequence"
          else if ((Char.code ch land 0x07) lsl 18)
                  lor ((byte1 land 0x3f) lsl 12)
                  lor ((byte2 land 0x3f) lsl 6)
                  lor (byte3 land 0x3f)
                  < 0x10000
          then i, ulen, "overlong UTF8 sequence"
          else main (i + 4) (ulen + 1))
      | _ -> i, ulen, "invalid start of UTF8 sequence")
  in
  main i 0
;;
...
let check str =
  ...
  then Core.Result.Ok len
  else Core.Result.Error (Printf.sprintf "at position %d: %s" ofs msg)
;;
...
let unsafe_next str ofs =
  ...
  | '\x00' .. '\x7f' -> ofs + 1
  | '\xc0' .. '\xdf' ->
    if ofs + 2 > String.length str
    then fail str ofs "unterminated UTF-8 sequence"
    else ofs + 2
  | '\xe0' .. '\xef' ->
    if ofs + 3 > String.length str
    then fail str ofs "unterminated UTF-8 sequence"
    else ofs + 3
  | '\xf0' .. '\xf7' ->
    if ofs + 4 > String.length str
    then fail str ofs "unterminated UTF-8 sequence"
    else ofs + 4
  | _ -> fail str ofs "invalid start of UTF-8 sequence"
;;
        ...
        then (
          ...
          | '\x80' .. '\xbf' ->
            ...
            then (
              ...
              | '\xf0' .. '\xf7' -> ofs - 4
              | _ -> fail str (ofs - 4) "invalid start of UTF-8 sequence")
            else fail str (ofs - 3) "invalid start of UTF-8 string"
          | _ -> fail str (ofs - 3) "invalid middle of UTF-8 sequence")
        else fail str (ofs - 2) "invaild start of UTF-8 string"
      | _ -> fail str (ofs - 2) "invalid middle of UTF-8 sequence")
    else fail str (ofs - 1) "invalid start of UTF-8 string"
  | _ -> fail str (ofs - 1) "invalid end of UTF-8 sequence"
;;

let unsafe_extract str ofs =
  let ch = String.unsafe_get str ofs in
  match ch with
  | '\x00' .. '\x7f' -> Zed_char.of_char ch
  | '\xc0' .. '\xdf' ->
    if ofs + 2 > String.length str
    then fail str ofs "unterminated UTF-8 sequence"
    else
      Zed_char.of_int (((Char.code ch land 0x1f) lsl 6) lor (byte str (ofs + 1) land 0x3f))
  | '\xe0' .. '\xef' ->
    if ofs + 3 > String.length str
    then fail str ofs "unterminated UTF-8 sequence"
    else
      Zed_char.of_int
        (((Char.code ch land 0x0f) lsl 12)
         lor ((byte str (ofs + 1) land 0x3f) lsl 6)
         lor (byte str (ofs + 2) land 0x3f))
  | '\xf0' .. '\xf7' ->
    if ofs + 4 > String.length str
    then fail str ofs "unterminated UTF-8 sequence"
    else
      Zed_char.of_int
        (((Char.code ch land 0x07) lsl 18)
         lor ((byte str (ofs + 1) land 0x3f) lsl 12)
         lor ((byte str (ofs + 2) land 0x3f) lsl 6)
         lor (byte str (ofs + 3) land 0x3f))
  | _ -> fail str ofs "invalid start of UTF-8 sequence"
;;
...
let unsafe_extract_next str ofs =
  let ch = String.unsafe_get str ofs in
  match ch with
  | '\x00' .. '\x7f' -> Zed_char.of_char ch, ofs + 1
  | '\xc0' .. '\xdf' ->
    if ofs + 2 > String.length str
    then fail str ofs "unterminated UTF-8 sequence"
    else
...
let unsafe_extract_prev str ofs =
  ...
  match ch1 with
  | '\x00' .. '\x7f' -> Zed_char.of_char ch1, ofs - 1
  | '\x80' .. '\xbf' ->
    if ofs >= 2
    then (
      ...
      match ch2 with
      | '\xc0' .. '\xdf' ->
        ( Zed_char.of_int (((Char.code ch2 land 0x1f) lsl 6) lor (Char.code ch1 land 0x3f))
        , ofs - 2 )
      | '\x80' .. '\xbf' ->
        if ofs >= 3
        then (
          ...
          | '\xe0' .. '\xef' ->
            ( Zed_char.of_int
                (((Char.code ch3 land 0x0f) lsl 12)
                 lor ((Char.code ch2 land 0x3f) lsl 6)
                 lor (Char.code ch1 land 0x3f))
            , ofs - 3 )
          | '\x80' .. '\xbf' ->
            if ofs >= 4
            then (
              ...
              | '\xf0' .. '\xf7' ->
                ( Zed_char.of_int
                    (((Char.code ch4 land 0x07) lsl 18)
                     lor ((Char.code ch3 land 0x3f) lsl 12)
                     lor ((Char.code ch2 land 0x3f) lsl 6)
                     lor (Char.code ch1 land 0x3f))
                , ofs - 4 )
              | _ -> fail str (ofs - 4) "invalid start of UTF-8 sequence")
...
let rec move_l str ofs len =
  if len = 0
  then ofs
  else if ofs = String.length str
  then raise Out_of_bounds
  else move_l str (unsafe_next str ofs) (len - 1)
;;

let unsafe_sub str ofs len =
  let res = Bytes.create len in
  Bytes.blit_string str ofs res 0 len;
...
let singleton char =
  let code = Zed_char.code char in
  if code < 0x80
  then (
...
let rec length_rec str ofs len =
  if ofs = String.length str then len else length_rec str (unsafe_next str ofs) (len + 1)
;;
...
let rec compare_rec str1 ofs1 str2 ofs2 =
  ...
  then if ofs2 = String.length str2 then 0 else -1
  ...
  then 1
    ...
    let d = Zed_char.code code1 - Zed_char.code code2 in
    if d <> 0 then d else compare_rec str1 ofs1 str2 ofs2)
;;
...
let rev str =
  let len = String.length str in
  Bytes.unsafe_to_string (rev_rec (Bytes.create len) str 0 len)
;;
...
let concat sep l =
  ...
  | x :: l ->
    ...
    let len =
      List.fold_left
        (fun len str -> len + sep_len + String.length str)
        (String.length x)
        l
    in
    let res = Bytes.create len in
...
let explode str = explode_rec str (String.length str) []
...
let rev_iter f str = rev_iter_rec f str (String.length str)
...
let rev_fold f str acc = rev_fold_rec f str (String.length str) acc
...
let map f str = map_rec (Buffer.create (String.length str)) f str 0
...
let map_concat f str = map_concat_rec (Buffer.create (String.length str)) f str 0
...
let rec filter_rec buf f str ofs =
  ...
  then Buffer.contents buf
    ...
    let chr, ofs = unsafe_extract_next str ofs in
    if f chr then Buffer.add_string buf (singleton chr);
    filter_rec buf f str ofs)
;;

let filter f str = filter_rec (Buffer.create (String.length str)) f str 0
...
let rec filter_map_rec buf f str ofs =
  ...
  then Buffer.contents buf
    ...
    let chr, ofs = unsafe_extract_next str ofs in
    (match f chr with
     | Some chr -> Buffer.add_string buf (singleton chr)
     | None -> ());
    filter_map_rec buf f str ofs)
;;

let filter_map f str = filter_map_rec (Buffer.create (String.length str)) f str 0
...
let rec filter_map_concat_rec buf f str ofs =
  ...
  then Buffer.contents buf
    ...
    let chr, ofs = unsafe_extract_next str ofs in
    (match f chr with
     | Some txt -> Buffer.add_string buf txt
     | None -> ());
...
let rec contains_rec str sub ofs =
  if ofs + String.length sub > String.length str
  then false
  else unsafe_sub_equal str ofs sub 0 || contains_rec str sub (unsafe_next str ofs)
;;
...
let starts_with str prefix =
  ...
  then false
  else unsafe_sub_equal str 0 prefix 0
;;
...
let rec lfind predicate str ofs =
  ...
  then ofs
    ...
    let chr, ofs' = unsafe_extract_next str ofs in
    if predicate chr then lfind predicate str ofs' else ofs)
;;
...
let rec rfind predicate str ofs =
  ...
  then 0
    ...
    let chr, ofs' = unsafe_extract_prev str ofs in
    if predicate chr then rfind predicate str ofs' else ofs)
;;

let strip ?(predicate = Zed_char.is_space) str =
  let lofs = lfind predicate str 0
  and rofs = rfind predicate str (String.length str) in
  if lofs < rofs then unsafe_sub str lofs (rofs - lofs) else ""
;;

let lstrip ?(predicate = Zed_char.is_space) str =
  let lofs = lfind predicate str 0 in
  unsafe_sub str lofs (String.length str - lofs)
;;

let rstrip ?(predicate = Zed_char.is_space) str =
...
let add buf char =
  let code = Zed_char.code char in
  if code < 0x80
  then Buffer.add_char buf (Char.unsafe_chr code)
  else if code <= 0x800
  then (
...
let encode_to_bigstring buf char ~pos =
  let code = Zed_char.code char in
  if code < 0x80
  then (
...
let extract str ofs =
  if ofs < 0 || ofs >= String.length str
  then raise Out_of_bounds
  else unsafe_extract str ofs
;;

let next str ofs =
  if ofs < 0 || ofs >= String.length str then raise Out_of_bounds else unsafe_next str ofs
;;

let extract_next str ofs =
  if ofs < 0 || ofs >= String.length str
  then raise Out_of_bounds
  else unsafe_extract_next str ofs
;;

let prev str ofs =
  if ofs <= 0 || ofs > String.length str then raise Out_of_bounds else unsafe_prev str ofs
;;
...
let add_escaped_char buf ch =
  match Zed_char.code ch with
  | 7 -> Buffer.add_string buf "\\a"
  | 8 -> Buffer.add_string buf "\\b"
  | 9 -> Buffer.add_string buf "\\t"
  | 10 -> Buffer.add_string buf "\\n"
  | 11 -> Buffer.add_string buf "\\v"
  | 12 -> Buffer.add_string buf "\\f"
  | 13 -> Buffer.add_string buf "\\r"
...
let add_escaped buf str = iter (add_escaped_char buf) str
...
let add_escaped_string buf str =
  ...
  while !ofs < String.length str do
    ...
    with
    | Invalid _ ->
      Printf.bprintf buf "\\y%02x" (Char.code str.[!ofs]);
      incr ofs
  done
;;
...
let sexp_of_t = Core.String.sexp_of_t

let t_of_sexp s =
  let t = Core.String.t_of_sexp s in
  ignore (validate t : int);
  t
;;
