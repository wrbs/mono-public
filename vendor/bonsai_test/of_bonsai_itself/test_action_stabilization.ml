...
module%test [@name "pruning"] _ = struct
  ...
  let%expect_test "old assoc branches are pruned after not receiving actions for a long \
                   time"
    =
  ...
  let%expect_test "old assoc_on branches are pruned after not receiving actions for a \
    ...
    =
    run_assoc_test
      (Bonsai.Expert.assoc_on (module Int) (module Unit) ~get_model_key:(fun _ () -> ()));
    [%expect
      {|
      After pruning:
      ((stabilizations_before_actions 5402)
  ...
  let%expect_test "old switch branches are pruned after not receiving actions for a long \
                   time"
    =
...
module%test [@name "interesting action delivery cases"] _ = struct
  let%expect_test "dynamic action being delivered to a switch that requires stabilization"
    =
  ...
  let%expect_test "dynamic action being delivered to an assoc that requires stabilization"
    =
    let component =
      let%sub input, set_input = Bonsai.state 0 in
      let%sub m =
