...
(** These are designed to test the generated path IDs, and shapes of [Computation.t] and
    [Incr.t]s produced by the proc and cont APIs.

    [basic] subs on a subcomputation [width] times, where the subcomputation is a chain of
    [height] unused [Bonsai.state]s.
    ...
    [with_switch] subs on [basic], then creates a [Switch] node with a constant arm, a
    simple `let%arr` arm, and an arm that's just [basic] again. *)
