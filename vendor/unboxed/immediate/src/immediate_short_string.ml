...
module Stable = struct
  module V1 = struct
    module T_stringable = struct
      ...
      let to_string__stack = to_local_string
    ...
    include%template
      Sexpable.Of_stringable.V1 [@alloc stack] [@modality portable] (T_stringable)
  ...
  module V1 = struct
    ...
    let%template[@alloc a = (heap, stack)] sexp_of_t t =
      (let o = to_or_null t in
       [%sexp (o : Stable.V1.t Core.Or_null.t)] [@alloc a])
      [@exclave_if_stack a]
    ;;
...
let of_i64 ~len t =
  let contents =
    (* we want to mask out to just the [len] low bytes, but you can't shift by 64, so
       start with a mask that is only the low 7 bytes and shift it more as needed. *)
    let shift = 56 - (len * 8) in
    let open I64.O in
    let mask = (-#1L lsr 8) lsr shift in
    t land mask
  in
  let t = unsafe_create len |> I64.of_int in
  I64.O.(t lor contents) |> I64.to_int_trunc
;;
