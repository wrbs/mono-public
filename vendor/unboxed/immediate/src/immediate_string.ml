    ...
    module V1 = struct
      module T_stringable = struct
        ...
        let%template[@alloc a = (heap, stack)] to_string t =
          (if is_interned t
           then (Interned_string.to_string [@alloc a]) (unsafe_to_interned t)
           else (Short_string.to_string [@alloc a]) (unsafe_to_short t))
          [@exclave_if_stack a]
        ;;
      ...
      include%template Sexpable.Of_stringable.V1 [@alloc stack] (T_stringable)
  ...
  module Option_stable = struct
    module V1 = struct
      ...
      let%template[@alloc a @ m = (heap_global, stack_local)] sexp_of_t t =
        (let o = (to_option [@mode m]) t in
         [%sexp (o : Stable.V1.t option)] [@alloc a])
        [@exclave_if_stack a]
      ;;
