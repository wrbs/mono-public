...
This conditional exclave will be replaced by future compiler features and so requires that
you specify which features it is emulating. This is done using an optional labelled
argument `~reasons`, for which a list of pre-defined reasons may be provided. Any
non-empty subset of these may be provided. A proper application's syntax looks like:
`[@exclave_if_local m ~reasons:[ Mode_polymorphism ]]`.
...
expression because we can identify syntactically that they will be supportd:
