...
let execute re a =
  apply_permutation
    ...
    (Array.map (fun i -> Re.exec re strings.(i)) a)
;;
...
let test compose n =
  ...
  for _ = 1 to n do
    ...
    try
      let groups, groups' = compose (fun () -> execute re a) (fun () -> execute re a') in
      let ok = Array.for_all2 compare_groups groups groups' in
      success := !success && ok;
