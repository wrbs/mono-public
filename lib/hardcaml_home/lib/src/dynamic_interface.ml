...
struct
  module Of_signal = struct
    ...
    let wires nbits =
      T.port_names_and_widths_dynamic ~nbits
      |> T.map ~f:(fun (_, width) -> Signal.wire width)
    ;;

    let ( -- ) t prefix =
      T.map2 T.port_names_and_widths t ~f:(fun (name, _) signal ->
        Signal.( -- ) signal [%string "%{prefix}$%{name}"])
    ;;
