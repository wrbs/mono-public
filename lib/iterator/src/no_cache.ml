...
let create (iter @ m) = { iter } [@exclave_if_stack a]
...
let length t =
  let length = ref 0 in
  iter t ~f:(fun _ -> length := !length + 1);
  !length
;;
...
let to_iarray ?size_hint t = to_vec ?size_hint t |> Vec.to_iarray
...
let cache_with_length ?size_hint (t @ local) =
  let vec = to_vec ?size_hint t in
  of_vec vec, ~length:(Vec.length vec)
;;

let of_iarray x = create @@ fun ~f -> Iarray.iter x ~f

let rev ?size_hint (t @ local) =
  let vec = to_vec ?size_hint t in
  create
  @@ fun ~f ->
  for idx = Vec.length vec - 1 downto 0 do
    f (Vec.unsafe_get vec idx)
  done
;;

let sort ?size_hint t ~compare =
  let vec = to_vec t ?size_hint in
  Vec.sort vec ~compare;
  of_vec vec
;;

let to_sequence ?size_hint t =
  let arr = to_vec ?size_hint t |> Vec.to_iarray in
  Iarray.to_sequence arr
;;
...
let group t ~break =
  (group' [@alloc a]) t ~break ~handle_vec:(fun vec -> of_iarray (Vec.to_iarray vec))
  [@exclave_if_stack a]
;;
...
let chunks_of t ~length =
  (chunks_of' [@alloc a]) t ~length ~handle_vec:(fun vec -> of_iarray (Vec.to_iarray vec))
  [@exclave_if_stack a]
;;
...
module Using_effects = struct
  module Unique_driver = struct
    ...
    type 'a t = (unit, unit, 'a, unit) E.Continuation.t
  ...
  module Driver = struct
    type 'a t = 'a Unique_driver.t option Unique.Ref.t

    let create iter =
      let driver = Unique_driver.create iter in
      Unique.Ref.make (Some driver)
    ;;

    let next t =
      match Unique.Ref.exchange t None with
      | None -> None
      | Some driver' ->
        (match Unique_driver.step driver' with
         | Done -> None
         | Next (value, driver') ->
           let (_ : _ Unique_driver.t option) = Unique.Ref.exchange t (Some driver') in
           Some value)
    ;;
  end
